...
def evaluate_all_admet_predictions(
    y_true: dict[str, np.ndarray], all_y_pred: dict[str, dict[str, np.ndarray]], rank_by, ascending=True
) -> pd.DataFrame:
    """
    ...
    leaderboards = scores_to_leaderboards(
        all_scores, rank_by="mean_absolute_error", ascending=ascending
    )
