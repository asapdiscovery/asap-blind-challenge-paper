...
\author{Jenke Scheen}
\affiliation{Open Molecular Software Foundation, Davis CA, USA}
\altaffiliation{CHARM Therapeutics, London, UK}
\author{Cas Wognum}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Data:
% Check very carefully for test-train overlap in your data package, it can hide almost anywhere
% You may not be able to split your compounds using conventional strategies. Eventually we settled on an index based time-biased split as a compromise between several competing needs (minimal overlap, minimal cross-challenge overlap, time wise splitting).
% Nasty hidden bits of data, for example
%  several activity cliffs were found along the way
% Several compounds registered with the same smiles were known to be unresolved enantiomers
% Extended stereochemistry is not well understood and not always reliable
% Several X ray structures were run on enantiomeric mixtures, these however are actually resolved in the structure themselves to the most potent compound
% Some thresholds and assay standards can shift over time, e.g Bienta HLM / MLM sensitivity was not properly marked at start. Good record keeping is essential.

% Data distributions can be 
% Community:
% Should we have been clearer on the fact that we were running a lead-opt focused challenge? Several people investigated and commented on the focused nature of the challenge.
% Poses has been technically challenging, lack of tactile feedback makes people debugging submissions hard
